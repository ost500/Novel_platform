...
<div class="container" xmlns:v-on="http://www.w3.org/1999/xhtml">
    <div class="cont_wrap" id="completed">
        <div class="sel2_wrap">
            <!-- 텝메뉴 -->
            <ul class="tap2_mn">
                <li class="left"><a href="{{route('completed')}}"
                                    @if(!$free_or_charged) class="tap2_mn_on" @endif>유료소설</a></li>
                <li class="right"><a href="{{route('completed',['free_or_charged'=>'free'])}}"
        ...
        <div class="sort_area_wrap">
            ...
            <div class="sort_area">
                <a href="{{route('completed',['free_or_charged'=>$free_or_charged])."?genre=".$genre}}"
        ...
        <!-- 페이징2 -->
        @include('pagination_mobile', ['collection' => $novel_groups, 'url' => route('completed',['free_or_charged'=>$free_or_charged])."?genre=".$genre."&order=".$order.'&'])
                <!-- 페이징2 //-->
    </div>
</div>
<!-- 내용 //-->
        ...
        methods: {
            callUrl: function () {
                ...
                if (this.optionValue == '전체') {
                    location.assign('{{route('completed')}}' + this.queryString);
                } else if (this.optionValue == '현대로맨스') {
                    location.assign('{{route('completed')}}' + this.queryString + '?genre=현대로맨스');
                } else if (this.optionValue == '시대로맨스') {
                    location.assign('{{route('completed')}}' + this.queryString + '?genre=시대로맨스');
                } else if (this.optionValue == '로맨스판타지') {
                    location.assign('{{route('completed')}}' + this.queryString + '?genre=로맨스판타지');
                }
            }
        }
    });
