...
<div id="my_info_bar" class="{{ Request::is('m/my_info')?"padt30":"sel2_wrap" }}" xmlns:v-on="http://www.w3.org/1999/xhtml">
    <select class="{{ Request::is('m/my_info/favorites')  || Request::is('m/my_info/personal/*') || Request::is('m/mails/*') ?"sel_198":"full" }}" id="myinfoSelect" v-on:change="callUrl()">
         ...
         <option value="개인" @if(Request::is('m/my_info/personal/*') || Request::is('m/mails/*')) selected @endif>개인</option>
     </select>
    ...
    @endif
  @if(Request::is('m/my_info/personal/*') || Request::is('m/mails/*'))
     <select class="sel_346 marL8" id="myinfo_favorites"  v-on:change="callSubUrl()">
        <option value="게시글 관리"  @if(Request::is('m/my_info/personal/post_manage')) selected @endif>게시글 관리</option>
        <option value="추천 리뷰 관리" @if(Request::is('m/my_info/personal/review_manage')) selected @endif>추천 리뷰 관리</option>
        <option value="소설 댓글 관리" @if(Request::is('m/my_info/personal/novel_comments_manage')) selected @endif>소설 댓글 관리</option>
        <option value="일반 댓글 관리" @if(Request::is('m/my_info/personal/free_board_review_comments_manage')) selected @endif>일반 댓글 관리</option>
        <option value="정보변경"  @if(Request::is('m/my_info/personal/edit') || Request::is('m/my_info/personal/password_again')) selected @endif>정보변경</option>
        <option value="쪽지"  @if(Request::is('m/mails/*')) selected @endif>쪽지</option>
      </select>
  @endif
   @if(Request::is('m/mails/*'))
  <select class="full" id="myinfo_mails"  v-on:change="callSubUrlMail()" style="margin-top:10px;">
      <option value="받은쪽지함"  @if(Request::is('m/mails/received')) selected @endif>받은쪽지함</option>
      <option value="보낸쪽지함"  @if(Request::is('m/mails/sent')) selected @endif>보낸쪽지함</option>
      <option value="보관쪽지함"  @if(Request::is('m/mails/my_box')) selected @endif>보관쪽지함</option>
      <option value="스팸쪽지함"  @if(Request::is('m/mails/spam')) selected @endif>스팸쪽지함</option>
      <option value="쪽지보내기"  @if(Request::is('m/mails/create') || Request::is('m/mails/create/*')) selected @endif>쪽지보내기</option>
  </select>
  @endif
</div>
                   ...
                   //Get the selected value
                    ...
                    //Based on values make a request
                     if (this.subOptionMail == '받은쪽지함') {
                        ...
                        location.assign('{{route('m.mails.sent')}}');
                    }else if (this.subOptionMail == '보관쪽지함') {
                        location.assign('{{route('m.mails.my_box')}}');
                    }else if (this.subOptionMail == '스팸쪽지함') {
                        location.assign('{{route('m.mails.spam')}}');
                    }else if (this.subOptionMail == '쪽지보내기') {
                        location.assign('{{route('m.mails.create')}}');
                    }
